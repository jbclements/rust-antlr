...
module_contents : extern_mod_view_item* view_item* mod_item*;
...
foreign_mod
  : EXTERN (LIT_STR)? MOD ident LBRACE foreign_item* RBRACE
  | EXTERN (LIT_STR)? LBRACE foreign_item* RBRACE ;
foreign_item
  : attrs STATIC ident COLON ty SEMI
  | attrs visibility (UNSAFE)? FN ident (LT (generic_decls)? GT)? LPAREN (args)? RPAREN ret_ty SEMI
  ;
...
fun_body : LBRACE view_item* block_element* (block_last_element)? RBRACE ;
...
attrs_and_vis : attrs visibility ;
...
attrs : /* nothing */ | attr attrs ;
attr : POUND LBRACKET meta_item RBRACKET (SEMI)?
  | OUTER_DOC_COMMENT
  | INNER_DOC_COMMENT;
meta_item : ident
  | ident EQ lit
